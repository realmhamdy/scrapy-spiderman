...
from spiderman.management.commands.collect_spiders import Command
from spiderman.models import Spider


class TestCollectSpidersCommand(TestCase):

    @mock.patch("spiderman.management.commands.collect_spiders.settings.SPIDER_DIRS", None, create=True)
    def test_raises_improperlyconfigured(self):
        self.assertRaises(ImproperlyConfigured, Command().handle)

    @mock.patch("spiderman.management.commands.collect_spiders.settings.SPIDER_DIRS",
